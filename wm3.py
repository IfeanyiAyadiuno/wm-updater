...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        # ========== Tab 1: Dashboard
        self.tab_dashboard = ttk.Frame(self.nb, style="Modern.TFrame")
        ...
        # Build dashboard (after colors are defined)
    ...
    def build_dashboard(self):
        """Build the dashboard tab with key metrics and visualizations."""
        # Clear existing widgets
        for widget in self.tab_dashboard.winfo_children():
            widget.destroy()
        
        # Create scrollable frame for dashboard
        dashboard_scroll = XYScrollFrame(self.tab_dashboard)
        dashboard_scroll.pack(fill="both", expand=True, padx=16, pady=16)
        
        # Main container
        container = ttk.Frame(dashboard_scroll.viewPort, style="Modern.TFrame")
        container.pack(fill="both", expand=True)
        
        # Title
        title_frame = ttk.Frame(container, style="Modern.TFrame")
        title_frame.pack(fill="x", pady=(0, 20))
        ttk.Label(title_frame, text="📊 WM Database Dashboard", 
                 style="Modern.TLabel", font=('Segoe UI', 16, 'bold')).pack(side="left")
        
        # Last updated label
        self.last_updated_label = ttk.Label(title_frame, text="", 
                                           style="Modern.TLabel", 
                                           font=('Segoe UI', 9),
                                           foreground=self.colors['text_muted'])
        self.last_updated_label.pack(side="left", padx=(20, 0))
        
        ttk.Button(title_frame, text="🔄 Refresh", command=self.refresh_dashboard, 
                  style="Modern.TButton").pack(side="right")
        
        # Stats Grid
        stats_frame = ttk.Frame(container, style="Modern.TFrame")
        stats_frame.pack(fill="x", pady=(0, 20))
        
        self.dashboard_stats = {}
        self.create_stat_card(stats_frame, "Total Wells", "0", "primary")
        self.create_stat_card(stats_frame, "Complete Wells", "0", "success")
        self.create_stat_card(stats_frame, "Pending Wells", "0", "warning")
        self.create_stat_card(stats_frame, "Staged for Add", "0", "secondary")
        
        # Charts Section
        charts_container = ttk.Frame(container, style="Modern.TFrame")
        charts_container.pack(fill="both", expand=True)
        
        # Left column - Top Formations
        left_col = ttk.Frame(charts_container, style="Modern.TFrame")
        left_col.pack(side="left", fill="both", expand=True, padx=(0, 10))
        
        ttk.Label(left_col, text="Top Formations", style="Modern.TLabel", 
                 font=('Segoe UI', 12, 'bold')).pack(anchor="w", pady=(0, 10))
        self.formation_list = tk.Text(left_col, height=12, wrap="none", 
                                      font=('Segoe UI', 9), bg=self.colors['surface'])
        self.formation_list.pack(fill="both", expand=True)
        
        # Right column - Top Pads
        right_col = ttk.Frame(charts_container, style="Modern.TFrame")
        right_col.pack(side="right", fill="both", expand=True, padx=(10, 0))
        
        ttk.Label(right_col, text="Top Pads", style="Modern.TLabel", 
                 font=('Segoe UI', 12, 'bold')).pack(anchor="w", pady=(0, 10))
        self.pad_list = tk.Text(right_col, height=12, wrap="none", 
                               font=('Segoe UI', 9), bg=self.colors['surface'])
        self.pad_list.pack(fill="both", expand=True)
        
        # Bottom section - Completions Technology
        bottom_frame = ttk.Frame(container, style="Modern.TFrame")
        bottom_frame.pack(fill="x", pady=(20, 0))
        
        ttk.Label(bottom_frame, text="Completions Technology Distribution", 
                 style="Modern.TLabel", font=('Segoe UI', 12, 'bold')).pack(anchor="w", pady=(0, 10))
        self.tech_frame = ttk.Frame(bottom_frame, style="Modern.TFrame")
        self.tech_frame.pack(fill="x")
        
        # Initial load (will be populated after first data load)
        # Dashboard will be refreshed automatically when data loads
    
    def create_stat_card(self, parent, title, value, color_type):
        """Create a modern stat card."""
        card = tk.Frame(parent, bg=self.colors['surface'], relief="flat", bd=1)
        ...
        value_label = tk.Label(content, text=value, 
                              ...
                              fg=self.colors[f'{color_type}_hover' if f'{color_type}_hover' in self.colors else 'primary'])
    ...
    def refresh_dashboard(self):
        ...
        # Complete wells (have Well Name)
        complete_mask = df["Well Name"].notna() & (df["Well Name"].astype(str).str.strip() != "")
        complete_wells = complete_mask.sum()
        pending_wells = total_wells - complete_wells
        staged_count = len(self.new_ids)
        
        # Update stat cards
        self.dashboard_stats["Total Wells"].config(text=str(total_wells))
        self.dashboard_stats["Complete Wells"].config(text=str(complete_wells))
        self.dashboard_stats["Pending Wells"].config(text=str(pending_wells))
        self.dashboard_stats["Staged for Add"].config(text=str(staged_count))
        
        # Top Formations
        self.formation_list.delete(1.0, tk.END)
        if "Formation Producer" in df.columns:
            formations = df["Formation Producer"].value_counts().head(10)
            for idx, (formation, count) in enumerate(formations.items(), 1):
                if pd.notna(formation) and str(formation).strip():
                    pct = (count / total_wells) * 100
                    self.formation_list.insert(tk.END, f"{idx:2d}. {str(formation)[:40]:<42} {count:>4} ({pct:.1f}%)\n")
        
        # Top Pads
        self.pad_list.delete(1.0, tk.END)
        if "Pad Name" in df.columns:
            pads = df["Pad Name"].value_counts().head(10)
            for idx, (pad, count) in enumerate(pads.items(), 1):
                if pd.notna(pad) and str(pad).strip():
                    pct = (count / total_wells) * 100
                    self.pad_list.insert(tk.END, f"{idx:2d}. {str(pad)[:40]:<42} {count:>4} ({pct:.1f}%)\n")
        
        # Completions Technology
        for widget in self.tech_frame.winfo_children():
            widget.destroy()
        
        if "Completions Technology" in df.columns:
            tech_counts = df["Completions Technology"].value_counts()
            total_tech = tech_counts.sum()
            
            for tech, count in tech_counts.head(8).items():
                if pd.notna(tech) and str(tech).strip():
                    pct = (count / total_tech) * 100
                    
                    # Tech label
                    tech_label = tk.Label(self.tech_frame, text=str(tech)[:30],
                                         font=('Segoe UI', 9),
                                         bg=self.colors['background'],
                                         fg=self.colors['text_primary'])
                    tech_label.pack(side="left", padx=(0, 8))
                    
                    # Progress bar background
                    bar_bg = tk.Frame(self.tech_frame, bg=self.colors['border'], height=20, width=200)
                    bar_bg.pack(side="left", padx=(0, 8))
                    
                    # Progress bar fill
                    bar_width = int(200 * (count / tech_counts.max()))
                    bar_fill = tk.Frame(bar_bg, bg=self.colors['primary'], height=20, width=bar_width)
                    bar_fill.pack(side="left", fill="y")
                    
                    # Count label
                    count_label = tk.Label(self.tech_frame, text=f"{count} ({pct:.1f}%)",
                                          font=('Segoe UI', 9),
                                          bg=self.colors['background'],
                                          fg=self.colors['text_secondary'],
                                          width=12, anchor="w")
                    ...
                    self.tech_frame.pack_configure(pady=4)
